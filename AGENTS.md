...
"pausa" / "salva aí". Not on a timer. Use the `session-checkpoint` skill.
...
document is incomplete. Use the `feature-doc` skill when writing or updating one.
...
Use the `project-readme` skill in those same cases.
...
Use the test conventions when working with test files; use the data-work conventions
when working with notebooks, SQL and pipelines.
