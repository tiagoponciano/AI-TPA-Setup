...
- **Codex:** global `~/.codex/AGENTS.md` (symlink to this repo's generated
  `AGENTS.md`); `~/.codex/hooks.json` includes the `PreToolUse` entry from
  `hooks/codex-hooks.json`; global Git `core.hooksPath` points at
  `~/.claude/hooks`; never add AI footers in commits or PR bodies
...
When asked, follow the PR message conventions in the appendix: conflict check, build and
test verification, then the description.

**Always, regardless of these conventions:** never merge the PR, never push to the base
...
"pausa" / "salva aí". Not on a timer. Follow the session checkpoint conventions in the appendix.
...
document is incomplete. Follow the feature documentation conventions in the appendix when writing or updating one.
...
follow the project README conventions in the appendix in those same cases.
...
Detailed conventions: test-file conventions and notebook/SQL/pipeline
conventions are covered separately below (auto-attached by glob where
...
| User asks for the PR | Follow PR message conventions (appendix) |
| Plan step done / pause / risky op / "checkpoint" | Follow session checkpoint conventions (appendix) |
| Any implementation | Update the feature doc, own commit — follow feature-doc conventions (appendix) |
| README missing, stale, or asked to improve | Follow project README conventions (appendix) |
